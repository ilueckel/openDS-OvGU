...
public class SimulatorActionListener implements ActionListener {
	...
	public void onAction(String binding, boolean value, float tpf) {
		...
		// note that our fancy car actually goes backwards..
			...
			if (value)
				sim.getCameraFactory().setShowBackViewMirror(
						!sim.getCameraFactory().getShowBackViewMirror());
		}
			...
			if (value) {
				car.getTransmission().setAutomatic(
						!car.getTransmission().isAutomatic());
			}
		}
			...
			if (value) {
				((SteeringCar) sim.getCar()).getObjectLocator()
						.rotateThingNode(-30);
			}
		}
