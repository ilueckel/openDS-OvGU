...
public class SceneLoader 
{
	...
	public SceneLoader(DrivingTaskDataQuery dtData, SimulationBasics sim) 
	{
		...
		getGeometries(new String[]{"box", "sphere", "cylinder"});
		getPoints();
		getResetPoints();
		mapObjectsList = createMapObjects();
	}
	 ...
	 * 
	 * @return
	 * 			List of additional map objects (dynamic, static spatial objects)
	 */
							...
							if(currentMaterialChild.getNodeName().equals("color"))
							{
								...
								if(color != null && spatial != null)
								{
									...
									if(materialNode != null)
										materialFile = materialNode.getNodeValue();

									if((materialFile == null) || (materialFile.equals("")))
										materialFile = "Common/MatDefs/Misc/Unshaded.j3md";
									
									Material material = new Material(sim.getAssetManager(), materialFile);
	...
	private Geometry createBox(String path, String name) 
	{
		...
		{
			// create new box
			Box box = new Box(new Vector3f(0, 0, 0), width, depth, height);
